...
vec2 points[10] = { {50, 400}, {100, 300}, {150, 250},
                    {200, 350}, {0,0}/*ez nem used*/, {370,210},
...
//double T[1] = 0.5;
//double T[2] = 1.5;
double T[3] = {-2.0, 0.5, 1.5};
double Tmin = *std::min_element(T,T+3);
double Tmax = *std::max_element(T,T+3);
//a bezierhez hasznalt ponthoz ki van rakva
vec2 E2BEZ;
//generaciok
...
void hermite(){
    ...
    vec3 Tvec = {static_cast<float>(T[0]), static_cast<float>(T[1]), static_cast<float>(T[2])};
    ...
    vec4 t2dif = {static_cast<float>(3*pow(T[2],2)),static_cast<float>(2*T[2]),1,0};
...
void bezier5() {
    ...
    for(double t = 0; t<=1; t+=0.01){
        ...
        for (int r = 1; r < 5; r++)
        {
            ...
            if ((t>(0.45+tolas)) && (t<(0.5+tolas)) && r==1){
                points_r1[0] = points_tmp[0];
                points_r1[1] = points_tmp[1];
                points_r1[2] = points_tmp[2];
                points_r1[3] = points_tmp[3];
                std::cout << t << std::endl;
            }

            if ((t>(0.45+tolas)) && (t<(0.5+tolas)) && r==2){
                points_r2[0] = points_tmp[0];
                points_r2[1] = points_tmp[1];
                points_r2[2] = points_tmp[2];
            }

            if ((t>(0.45+tolas)) && (t<(0.5+tolas)) && r==3){
                points_r3[0] = points_tmp[0];
                points_r3[1] = points_tmp[1];

            }

            if ((t>(0.45+tolas)) && (t<(0.5+tolas)) && r==4){
                points_r4 = points_tmp[0];
    ...
    //kiiratasok elegge csunya modon, de mukodik.
    glBegin(GL_LINE_STRIP);
...
void init()
{
    ...
    M ={{static_cast<float>(pow(T[0],3.0)),static_cast<float>(pow(T[1],3.0)),static_cast<float>(pow(T[2],3.0)),static_cast<float>(3*pow(T[0],2.0))},
        {static_cast<float>(pow(T[0],2.0)),static_cast<float>(pow(T[1],2.0)),static_cast<float>(pow(T[2],2.0)),static_cast<float>(2*pow(T[0],1.0))},
        {static_cast<float>(pow(T[0],1.0)),static_cast<float>(pow(T[1],1.0)),static_cast<float>(pow(T[2],1.0)),static_cast<float>(1)},
        {static_cast<float>(pow(T[0],0)),static_cast<float>(pow(T[1],0)),static_cast<float>(pow(T[2],0)),static_cast<float>(0)}};
