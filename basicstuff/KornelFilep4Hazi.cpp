...
void haziko(){
    ...
    tetopont = TMP(T3-0.075)*35  + tetoalap;
    //Hazikonak a teteje
    glColor3f(.5, 0.5, 0.0);
    glBegin(GL_POLYGON);
    ...
    ablakpont1 = TMP(T3-0.05) * 8 + ablakalap1;
    ablakpont2 = TMP(T3-0.05) * 15 + ablakalap1;

    vec2 ablakpont3;
    vec2 ablakpont4;
    vec2 ablakalap2 = hermitePoint(T3 - 0.09);
    ablakpont4 = ablakpont3;
    ablakpont3 = TMP(T3-0.09) * 8 + ablakalap2;
    ablakpont4 = TMP(T3-0.09) * 15 + ablakalap2;
